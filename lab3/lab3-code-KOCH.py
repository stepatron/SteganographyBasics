...
img_encode_path = 'img_encode.bmp'
block_size = 8
...
def can_encode(msg_plain, block_size, img_plain_path):
	...
	if len(msg_plain)*7+7 > (img_res[0]*img_res[1]/(block_size**2))*3:
		return -1
	return 0
...
def strEncode(msg_plain):
	...
	for pix_num in msg_plain:
		if pix_num == ' ': msg_cypher += '1111111'
		else: msg_cypher += format(ord(pix_num)-1024, '07b')
	msg_cypher += '0000000'
	return msg_cypher
		...
		for column_num in range(0, img_res[0], block_size):
			for row_num in range(0, img_res[0]*img_res[1], img_res[0]):
				...
				for pix_num in range(block_size):
					row_temp.append(pixels[column_num+row_num+pix_num][color_id])
...
def blocksToTuple(blocks, img_res, block_size):
	...
	column_c = row_c = block_c = pix_c = 0
...
def enDCT(block_plain):
	...
	for k in range(len(block_plain)):
		...
		for l in range(len(block_plain)):
			...
			if k==0 and l==0: alpha = 1
			elif k==0 or l==0: alpha = math.sqrt(2)
			else: alpha = 2
...
def encode(msg_plain, block_size, img_plain_path, img_encode_path):
	...
	delta = 5
	for bit in range(len(msg_cypher)):
		if bit < len(msg_cypher):
			if msg_cypher[bit] == '0':
				if blocks_enDCTed[bit][2][5] >= blocks_enDCTed[bit][5][2]:
					blocks_enDCTed[bit][2][5] = blocks_enDCTed[bit][5][2]-delta
				elif blocks_enDCTed[bit][5][2]-blocks_enDCTed[bit][2][5] < delta:
					blocks_enDCTed[bit][2][5] -= delta
			elif msg_cypher[bit] == '1':
				if blocks_enDCTed[bit][2][5] < blocks_enDCTed[bit][5][2]:
					blocks_enDCTed[bit][2][5] = blocks_enDCTed[bit][5][2]+delta
				elif blocks_enDCTed[bit][2][5]-blocks_enDCTed[bit][5][2] < delta:
					blocks_enDCTed[bit][2][5] += delta
	# Обратное ДКП
	blocks_deDCTed = []
...
def decode(img_encode_path, block_size):
	...
	for block in blocks_enDCTed:
		if block[2][5] < block[5][2]:
			msg_cypher += '0'
			zero_count += 1
			bit_count += 1
		elif block[2][5] > block[5][2]:
			msg_cypher += '1'
			bit_count += 1
...
if option == '1':
	...
	print('Встроено сообщение \"', msg_plain, '\" в изображение \"', img_encode_path, '\"', sep='')
